...
def _process_resume_task(resume_id: int, file_bytes: bytes, filename: str, content_type: str):
    db = SessionLocal()
    try:
        ...
        try:
            ...
            if profile:
                ...
                profile.phone = parsed.get("phone") or profile.phone
                profile.email = parsed.get("email") or profile.email
                profile.address = parsed.get("address") or profile.address
