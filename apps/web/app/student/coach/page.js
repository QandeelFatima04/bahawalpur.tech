...
function CoachView() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // First visit — conversation created on first send
      }
    }
    init();
  }, []);
  ...
  const playTTS = useCallback(
    async (text) => {
      ...
      try {
        ...
        setVadState("idle");
        // Re-activate VAD if still in voice mode
        if (voiceModeRef.current) vad.start();
      }
    },
        ...
        while (true) {
          ...
          for (const part of parts) {
            ...
            try {
              ...
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === asstMsgId ? { ...m, content: fullReply } : m
                )
              );
            } catch {}
          }
  ...
  const vadErrored = vad.errored;

  return (
    <div className="flex h-[calc(100vh-200px)] min-h-[480px] flex-col rounded-2xl bg-card shadow-sm ring-1 ring-black/5 overflow-hidden">
      {/* Messages area */}
